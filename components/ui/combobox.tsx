        ...
        >
            ...
            {shouldShowClear ? (
                <ComboboxClear
                    {...clearProps}
                    className={cn(
                        "absolute top-1/2 inline-flex size-8 shrink-0 -translate-y-1/2 cursor-pointer items-center justify-center rounded-md border border-transparent opacity-80 outline-none transition-opacity pointer-coarse:after:absolute pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11 hover:opacity-100 hover:transition-none has-[+[data-slot=combobox-clear]]:hidden sm:size-7 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
...
export function ComboboxGroupLabel({
    ...
    return (
        <ComboboxPrimitive.GroupLabel
            {...props}
            className={cn(
                "px-2 pt-1.5 pb-1 font-medium text-muted-foreground text-xs",
                className
            )}
